...
function lowerCaseStrings(strings) {
  strings.forEach((x, i) => strings[i] = x.toLowerCase());
  return strings; //or you could just use map and do it in 1 line ---- return strings.map(x => x.toLowerCase());
}
...
function removeApple(strArr) {
  return strArr.filter(x => x !== 'apple');
}
...
*/
function stringSmash(strArr) {
  return strArr.reduce((a, x) => a + x);
}
...
function tallyUpDonations(runners) {
  return runners.reduce((a, x) => (a + x.donation), 0);
}
 ...
 *  counter1 and counter2 do the same thing but reside in different lexical enviroments
 * 2. Which of the two uses a closure? How can you tell?
 *  they're both closures the only difference is that counter1 has a defined lexical enviroment (ConuterMaker) while counter2 uses the entire document.
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better? 
 *  counter1 would be preferable if you would like to make multiple counters and keep track of different numbers. counter2 might be preferable if you're keeping track of a global counter.
*/
